...
*   **書き込みプロセス**:
    ...
    4.  保存して閉じる
    5.  ロック解除
    6.  全データを再ロード（`loadAllData`）
    7.  UIの再描画（現在の患者を再選択する `selectPatient` の呼び出し等）を行い、変更を即座に画面に反映させる
...
*   **DO (やるべきこと)**:
    ...
    *   **保存後のUI同期**: データの書き込み（Excelへの保存）を行った後は、必ず `loadAllData()` を呼び出してメモリ上のデータを更新し、続けて適切なUI描画関数（`selectPatient()` 等）を呼び出して画面を最新状態に同期させること。これを怠ると、ユーザーが「保存内容が反映されない」と誤認する原因になります。
