...
### 1. エンコーディング
- HTAファイルは **Shift_JIS（Windows標準 / ANSI）** エンコーディングで保存すること
- UTF-8で保存するとアプリ上で日本語が文字化けする
- **注意**: VS Codeなどの近代的なエディタで開くと日本語が文字化けして見えることがあるが、これは正常。エディタ側で「エンコーディング付きで再度開く」→「Japanese (Shift JIS)」を選択すれば正しく表示される。
